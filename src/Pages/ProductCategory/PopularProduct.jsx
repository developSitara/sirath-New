...
const PopularProduct = ({ handleAddToCart }) => {
    ...
    <div className="w-full  mt-10  relative">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-6">
        <div className="text-center md:text-left">
          <h2 className="text-3xl font-bold capitalize">Popular Product</h2>
        </div>
      </div>
      ...
      <div className="mb-10 mt-10 overflow-x-auto flex space-x-4  w-full md:w-auto justify-around">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={`text-xs xl:text-lg relative px-3 py-1 xl:px-4 xl:py-2 xl:font-semibold transition-all duration-300 ${
              activeTab === tab.id
                ? "bg-blue text-white"
        ...
        >
          {filterProducts.length > 0 ? (
            filterProducts.map((item) => (
              <div className="mb-5 px-4 md:px-2 lg:px-2 xl:px-2" key={item.id}>
