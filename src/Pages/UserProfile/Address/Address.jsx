...
const addresses = [
  ...
  },
  {
    ...
    name: "Pooja Verma",
    phone: "9876545678",
    address: "Vill- Behror post Neemrana Alwar Rajasthan",
    pincode: "202303",
  },
];
...
const Address = () => {
  ...
  const isAddingNew = location.pathname.includes("add-new-address");

  return (
    <div className="w-full">
      {/* Show "Add New Address" button only when NOT in Add Address form */}
      {!isAddingNew && (
        <NavLink
          to="add-new-address"
      ...
      {/* Show only form when adding a new address */}
      {isAddingNew ? (
        <Outlet />
      ) : (
          ...
          {addresses.map((address) => (
            ...
            >
              ...
              <div className="absolute top-4 right-4">
                <FaEdit className="text-blue-600 cursor-pointer size-5" />
              </div>
              <div className="absolute bottom-4 right-4">
                <MdDelete className="text-red-700 cursor-pointer size-5" />
              </div>
            </div>
          ))}
        </div>
